  ...
  test("Should handle 100 levels of nesting", () => {
    ...
    for (let i = depth; i >= 0; i--) {
      ...
      if (i > 0) {
        ...
        const contentRaw = current.children.find((c: any) => c.isRaw() && c.toString().trim() === "content");
...
describe("TinyML Limits - Large Input", () => {
    test("Should handle large raw text (1MB)", () => {
        ...
        const raw = element.children!.find(c => c.isRaw() && c.toString().trim() === largeString) as Core.Raw;
